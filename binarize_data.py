...
from fairseq.data import indexed_dataset, dictionary, flexible_dictionary
...
def get_parser():
    ...
    parser.add_argument('-p', '--pointer-lang', default=None, metavar='POINTER', help='Pointer suffix')
...
def main(args):
    ...
    def make_pointer_dataset(input_prefix, output_prefix, lang):
        print('| [{}] do not need a dictionary'.format(lang))
        import torch
        ...
        for line in open(input_file, encoding='utf8'):
            ...
            idxs = None
            # convert to IntTensor
            if fds[0].isdigit():
                idxs = map(int, fds)
            else:
                assert fds[0] == 'T' or fds[0] == 'F'
                idxs = [i for i, fd in enumerate(fds) if fd == 'T']
                # print(idxs)

            tensor = torch.IntTensor(idxs)
            ds.add_item(tensor)

        ds.finalize(dataset_dest_path(output_prefix, lang, 'idx'))

    def make_dataset(input_prefix, output_prefix, lang):
        if lang == args.pointer_lang:
            assert args.output_format == 'binary'
            make_pointer_dataset(input_prefix, output_prefix, lang)
            return
    ...
    if args.pointer_lang is not None:
        make_all(args.pointer_lang)
