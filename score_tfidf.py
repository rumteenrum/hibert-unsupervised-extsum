...
def calculate_pmi_and_redundancy(all_targets_for_pmi, dataset, trainer=None):
    """Calculate similarity using TF-IDF vectors for both relevance and redundancy"""
    
    r_doc_pmi_relevance = []
    r_doc_pmi_redundancy = []
    
    for doc_idx, doc_targets in enumerate(all_targets_for_pmi):
        print(f"Processing TF-IDF similarity for document {doc_idx}")
        ...
        if not any(doc_sentences_text) or len([s for s in doc_sentences_text if s.strip()]) < 2:
            r_doc_pmi_relevance.append([0.0] * len(doc_targets))
            r_doc_pmi_redundancy.append([0.0] * len(doc_targets))
            continue
        
        # Filter out empty sentences for TF-IDF processing
        valid_sentences = [s if s.strip() else "empty" for s in doc_sentences_text]
        ...
        try:
            ...
            # ===== RELEVANCE using TF-IDF SIMILARITY to DOCUMENT =====
            # Document centroid (average of all sentence vectors)
            doc_centroid = np.mean(tfidf_matrix.toarray(), axis=0).reshape(1, -1)
            ...
            for sent_idx, sentence_text in enumerate(doc_sentences_text):
                ...
                sent_vector = tfidf_matrix[sent_idx].toarray()
                
                # Calculate cosine similarity with document centroid
                similarity = cosine_similarity(sent_vector, doc_centroid)[0][0]
                
                # Convert to log score (like PMI)
                relevance_score = math.log(max(similarity + 1, 1e-8))
                sent_relevance_scores.append(relevance_score)
            
            # ===== REDUNDANCY using TF-IDF SIMILARITY to PREVIOUS SENTENCES =====
            ...
            for sent_idx, sentence_text in enumerate(doc_sentences_text):
                ...
                # Get current sentence TF-IDF vector
                sent_vector = tfidf_matrix[sent_idx].toarray()
                
                # Get previous sentences vectors
                prev_vectors = tfidf_matrix[:sent_idx].toarray()
                
                if len(prev_vectors) > 0:
                    # Calculate similarity with all previous sentences
                    similarities = cosine_similarity(sent_vector, prev_vectors)[0]
                    max_similarity = np.max(similarities)
                    
                    # Convert to log score (higher similarity = higher redundancy)
                    redundancy_score = math.log(max(max_similarity + 1, 1e-8))
                else:
            ...
            print(f"TF-IDF error for document {doc_idx}: {e}")
            sent_relevance_scores = [0.0] * len(doc_targets)
            sent_redundancy_scores = [0.0] * len(doc_targets)
