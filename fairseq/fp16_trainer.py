...
class FP16Trainer(Trainer):
    ...
    def _all_reduce_and_rescale(self, grad_denom):
        ...
        if self.args.distributed_world_size > 1:
            ...
            if flat_grads.dtype != torch.float32:
            ...
            # single worker
            flat_grads = self._get_flat_grads().float()
            self.fp32_params.grad.data.copy_(flat_grads)
        ...
        if overflow:
            min_loss_scale = getattr(self.args, 'min_loss_scale', 1.0)
            if self.scaler.loss_scale <= min_loss_scale:
                raise Exception((
                    'Minimum loss scale reached ({}). Your loss is probably exploding. '
                    'Try lowering the learning rate, using gradient clipping or '
                    'increasing the batch size.'
                ).format(min_loss_scale))
