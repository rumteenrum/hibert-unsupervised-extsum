...
def replace_unk(hypo_str, src_str, alignment, align_dict, unk, attention=None, tgt_dict=None):
    ...
    for i, ht in enumerate(hypo_tokens):
        if ht == unk:
            ...
            # print('hypo: ', hypo_str)
            src_idx = alignment[i]
            # the attention of transformer is not accurate
            # we replace UNK word in the source, which has the highest attention score
            # assumming a known word can always been generated
            if attention  is not None:
                # exclude <eos>
                cur_attn = attention[0:-1, i]
                _, attn_idxs = cur_attn.sort(descending=True)
                for attn_idx in attn_idxs:
                    if tgt_dict.index( src_tokens[attn_idx] ) == tgt_dict.unk():
                        src_idx = attn_idx
                        break
...
def post_process_prediction(hypo_tokens, src_str, alignment, align_dict, tgt_dict, remove_bpe, attention=None):
    from fairseq import tokenizer
    hypo_str = tgt_dict.string(hypo_tokens, remove_bpe)
    if align_dict is not None:
        hypo_str = replace_unk(hypo_str, src_str, alignment, align_dict, tgt_dict.unk_string(), attention=attention, tgt_dict=tgt_dict)
