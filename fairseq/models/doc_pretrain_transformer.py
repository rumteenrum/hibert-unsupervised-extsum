...
class DocPretrainTransformerModel(FairseqModel):
    ...
    def forward(self, src_tokens, doc_pad_mask, doc_pos_tok, masked_sent_positions, prev_output_tokens):
        ...
        # Get attention from encoder output
        attention = encoder_out.get('last_layer_attention', None)
    ...
    def get_normalized_probs(self, net_output, log_probs, idx=0):
        ...
        # Handle the case when net_output is a tuple of two elements (decoder_out, attention)
        # or when it's just the decoder_out itself
        if isinstance(net_output, tuple) and len(net_output) == 2 and isinstance(net_output[0], tuple):
            # This is the case where we're passing the full model output (decoder_out, attention)
            decoder_out = net_output[0]
            logits = decoder_out[idx].float()
        else:
            # This is the case where we're only passing the decoder_out
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, doc_pad_mask, doc_pos_tok):
        ...
        for layer in self.layers:
            # We'll discard attention weights from the word-level encoder
        ...
        # Store attention matrices from document-level encoding
        sent_attentions = []
        for doc_layer in self.doc_layers:
            sent_repr, attn_weights = doc_layer(sent_repr, doc_pad_mask)
            sent_attentions.append(attn_weights)

        # Last layer attention is often most useful for summarization
        last_layer_attention = sent_attentions[-1] if sent_attentions else None

        return {
            'encoder_out': sent_repr,  # n_sent x bsz x C
            'encoder_padding_mask': doc_pad_mask,  # bsz x n_sent
            'last_layer_attention': last_layer_attention,  # Last layer attention weights
        }
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_padding_mask):
        ...
        # Get attention weights but don't disrupt the return type
        ...
        # Return both x and attention weights, but in a way that doesn't disrupt existing code
