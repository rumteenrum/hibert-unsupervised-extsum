...
class BERTModel(FairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        return BERTModel(encoder, decoder)

    def forward(self, src_tokens, span_ids, masked_positions):
        encoder_out = self.encoder(src_tokens, span_ids)
        decoder_out = self.decoder(encoder_out, masked_positions)
        return decoder_out
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=False):
        ...
        self.embed_span_id = nn.Embedding(2, embed_dim)

    def forward(self, src_tokens, span_ids):
        # embed tokens and positions
        x = self.embed_scale * self.embed_tokens(src_tokens)
        x += self.embed_positions(src_tokens)
        x += self.embed_span_id(span_ids)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary):
        ...
        self.embed_out_layer = Linear(embed_dim, len(dictionary))
        self.next_sent_out = Linear(embed_dim, 2)

    def forward(self, encoder_out, masked_positions):
        x = encoder_out['encoder_out']
        # T x B x C -> B x T x C
        x = x.transpose(0, 1)
        masked_enc_repr = get_sent_end_repr(x, masked_positions)
        label_pred = self.embed_out_layer(masked_enc_repr)
        sent_pred = self.next_sent_out(x[:, 0, :])

        return label_pred, sent_pred
...
@register_model_architecture('bert', 'bert_iwslt_de_en')
...
@register_model_architecture('bert', 'bert_wmt_en_de')
def transformer_wmt_en_de(args):
    base_architecture(args)


# parameters used in the "Attention Is All You Need" paper (Vaswani, et al, 2017)
@register_model_architecture('bert', 'bert_vaswani_wmt_en_de_big')
...
@register_model_architecture('bert', 'bert_vaswani_wmt_en_fr_big')
def transformer_vaswani_wmt_en_fr_big(args):
    args.dropout = getattr(args, 'dropout', 0.1)
    transformer_vaswani_wmt_en_de_big(args)


@register_model_architecture('bert', 'bert_wmt_en_de_big')
...
@register_model_architecture('bert', 'bert_wmt_en_de_big_t2t')
