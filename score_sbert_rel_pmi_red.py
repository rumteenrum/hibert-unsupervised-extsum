...
def calculate_pmi_and_redundancy(all_targets_for_pmi, dataset, trainer=None):
    """Calculate PMI using cosine similarity for relevance and token overlap for redundancy"""
    
    # Initialize sentence transformer model for relevance
    ...
    for doc_idx, doc_targets in enumerate(all_targets_for_pmi):
        ...
        # Convert targets to text sentences AND extract token lists
        doc_sentences_text = []
        ...
        for sent_idx, sent_tensor_np in enumerate(doc_targets):
            ...
            if tokens:
                # Convert token IDs back to text for embeddings
                text = ' '.join([dataset.src_dict[token] for token in tokens if token < len(dataset.src_dict)])
                doc_sentences_text.append(text)
                doc_sentences_tokens.append(tokens)  # Keep tokens for overlap calculation
            else:
                doc_sentences_text.append("")
                doc_sentences_tokens.append([])
        ...
        # ===== REDUNDANCY using TOKEN OVERLAP (from traincopy6) =====
        # Build document vocabulary for P_LM(d)
        all_doc_tokens = []
        for sent_tokens in doc_sentences_tokens:
            all_doc_tokens.extend(sent_tokens)
        ...
        for sent_idx, sentence_tokens in enumerate(doc_sentences_tokens):
            ...
            for prev_idx in range(sent_idx):
                prev_sentences.extend(doc_sentences_tokens[prev_idx])
            ...
            sent_vocab = set(sentence_tokens)
            ...
            sent_prob = sum(doc_vocab_counts.get(token, 1) / doc_total_tokens for token in sentence_tokens) / len(sentence_tokens) if sentence_tokens else 1e-8
