...
def main2(args):
    ...
    n = 4   # n-gram length for relevance and redundancy
    ...
    r_doc_pmi_relevance, r_doc_pmi_redundancy = calculate_pmi_and_redundancy(all_targets_for_pmi, dataset, n)
...
def calculate_pmi_and_redundancy(all_targets_for_pmi, dataset, n=4, trainer=None):
    """Calculate similarity using n-gram overlap for both relevance and redundancy"""
    
    r_doc_pmi_relevance = []
    r_doc_pmi_redundancy = []
    
    for doc_idx, doc_targets in enumerate(all_targets_for_pmi):
        print(f"Processing n-gram similarity for document {doc_idx}")
        
        # Convert targets to token lists
        doc_sentences_tokens = []
        for sent_idx, sent_tensor_np in enumerate(doc_targets):
            ...
            doc_sentences_tokens.append(tokens if tokens else [])
        
        if not any(doc_sentences_tokens):
            r_doc_pmi_relevance.append([0.0] * len(doc_targets))
            r_doc_pmi_redundancy.append([0.0] * len(doc_targets))
            continue
        
        # ===== RELEVANCE using N-GRAM SIMILARITY to DOCUMENT =====
        # Create document n-grams from all sentences
        all_doc_ngrams = set()
        for sent_tokens in doc_sentences_tokens:
            if sent_tokens:
                sent_ngrams = get_ngrams(sent_tokens, n)
                all_doc_ngrams.update(sent_ngrams)
        
        sent_relevance_scores = []
        for sent_idx, sentence_tokens in enumerate(doc_sentences_tokens):
            if not sentence_tokens:
                sent_relevance_scores.append(0.0)
                continue
            
            # Get sentence n-grams
            sent_ngrams = set(get_ngrams(sentence_tokens, n))
            
            # Calculate Jaccard similarity with document
            if sent_ngrams and all_doc_ngrams:
                intersection = len(sent_ngrams.intersection(all_doc_ngrams))
                union = len(sent_ngrams.union(all_doc_ngrams))
                jaccard_similarity = intersection / union if union > 0 else 0.0
                
                # Convert to log score (like PMI)
                relevance_score = math.log(max(jaccard_similarity + 1, 1e-8))
            else:
                relevance_score = 0.0
            
            sent_relevance_scores.append(relevance_score)
        
        # ===== REDUNDANCY using N-GRAM SIMILARITY to PREVIOUS SENTENCES =====
        sent_redundancy_scores = []
        for sent_idx, sentence_tokens in enumerate(doc_sentences_tokens):
            if not sentence_tokens or sent_idx == 0:
                sent_redundancy_scores.append(0.0)
                continue
            
            # Get current sentence n-grams
            sent_ngrams = set(get_ngrams(sentence_tokens, n))
            
            # Get previous sentences n-grams
            prev_ngrams = set()
            for prev_idx in range(sent_idx):
                if doc_sentences_tokens[prev_idx]:
                    prev_ngrams.update(get_ngrams(doc_sentences_tokens[prev_idx], n))
            
            if sent_ngrams and prev_ngrams:
                # Calculate Jaccard similarity with previous sentences
                intersection = len(sent_ngrams.intersection(prev_ngrams))
                union = len(sent_ngrams.union(prev_ngrams))
                jaccard_similarity = intersection / union if union > 0 else 0.0
                
                # Convert to log score (higher similarity = higher redundancy)
                redundancy_score = math.log(max(jaccard_similarity + 1, 1e-8))
            else:
                redundancy_score = 0.0
            
            sent_redundancy_scores.append(redundancy_score)
